...
Status: **Phase 0 in progress; the defined KDBX accepted/adversarial corpus gate, opaque generation-checked handle registry, Fortress credential boundary, engine-owned secret-memory/zeroization gate, concrete Rust vault-owner lifecycle, executable Android/Kotlin JNI caller, and bounded `open`/`lock`/`is-valid` JNI lifecycle are complete. The Android emulator now proves a real KDBX `open → is-valid → lock → stale` round trip while decrypted database ownership remains inside Rust. The next security-critical tranche is dedicated JNI/lifecycle hardening: panic containment, invalid/stale-handle behavior, Android lifecycle-triggered locking, then concurrency/property/fuzz coverage before any metadata or secret-retrieval API.**
...
- [x] Extend the JNI contract beyond the proven non-secret smoke boundary only after the executable Android/Kotlin caller passes; ABI v2 is intentionally limited to bounded lifecycle operations and still exposes no metadata, secret retrieval, mutation, persistence or networking API.
...
1. **Production Android vault use is blocked on lifecycle/error-boundary hardening, not on basic JNI reachability.** The bounded ABI-v2 wrapper now proves real Android `open`/`is-valid`/`lock` behavior while decrypted databases remain Rust-owned; dedicated panic-containment, invalid/stale-handle and Android lifecycle-triggered lock paths still require explicit proof before broader read APIs are allowed.
2. **Metadata/secret retrieval and production Android UI wiring remain blocked on completion of the lifecycle hardening gate and subsequent concurrency/property/fuzz coverage.** No broader JNI read surface is allowed until those gates preserve byte-oriented credentials, Rust-only decrypted-state ownership, opaque handles, sanitized errors and explicit lock semantics under failure and lifecycle transitions.
