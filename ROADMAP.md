...
- [ ] Research and select a free, redistribution-safe, high-quality and extensive icon source/library for folders, groups, entries and other UI objects. Evaluate license, visual consistency, coverage, Android suitability and offline bundling; require searchable metadata/tags/categories suitable for fast icon search and deterministic name/category matching (for example suggesting icons from folder/group/entry names) while always allowing manual selection.
