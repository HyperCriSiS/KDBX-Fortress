...
`kdbx4-empty-edge.kdbx` is a deterministic KDBX 4.1 Argon2id/AES-256-CBC edge-case fixture generated by the test-only Rust serializer helper. It distinguishes present-but-empty values from absent optional fields, includes an entirely field-less entry and an empty group, and is hash-locked in `manifest.json`.
